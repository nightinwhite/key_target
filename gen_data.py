...
if os.path.exists("{0}/slice_imgs_angle/".format(save_path)) == False:
    os.mkdir("{0}/slice_imgs_angle/".format(save_path))
if os.path.exists("{0}/slice_box_anns_angle/".format(save_path)) == False:
    os.mkdir("{0}/slice_box_anns_angle/".format(save_path))
if os.path.exists("{0}/slice_class_anns_angle/".format(save_path)) == False:
    os.mkdir("{0}/slice_class_anns_angle/".format(save_path))
base_img_w = 256
base_img_h = 256
splice_num = 4
wrong_anns_num = 0
...
def gen_single_data(img_path, anno_path):
    ...
    for ann_idx, ann in enumerate(anns):
        ...
        for s in range(splice_num):
            ...
            np.save("{0}/slice_class_anns_angle/{1}.npy".format(save_path, tmp_save_name), np.asarray(tmp_class_anns))
            # print(tmp_box_anns)
